...
import { percentDiffVsBaselineRoute } from './route-comparison.js';

describe('percentDiffVsBaselineRoute (Compute comparison)', () => {
  it('matches ((comparison / baseline) - 1) × 100', () => {
  ...
  it('rejects non-positive baseline', () => {
    ...
    expect(() => percentDiffVsBaselineRoute(1, -1)).toThrow();
  });
});
